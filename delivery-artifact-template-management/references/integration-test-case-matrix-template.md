...
- Fixed-height navigation/card/KPI fit: padding, explicit line-height, gaps, badge/status/footer heights, `requiredContentHeight <= componentHeight`, and DOM checks where `scrollHeight <= clientHeight` and `scrollWidth <= clientWidth` at `1920x1080`.
