...
| Filter task | Read |
| --- | --- |
| Screenshot/sample-derived filter style generalization | `report-component-style-design` `references/00a-style-generalization-goal.md` plus `$artifact-readability-standard` `references/visual-source-abstraction-standard.md` |
| Page/global query bar, filter group, date/org/status controls, advanced filter popover | `report-component-style-design` `references/02-filter-controls.md` |
| Reusable filter selector patterns, simple dropdown, tag multi-select, date range, searchable select, tree path selector, advanced drawer, combined chipbar | `references/02-filter-selector-patterns.md` plus `report-component-style-design` `references/02-filter-controls.md` |
