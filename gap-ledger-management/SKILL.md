...
description: "[原型阶段] 本阶段版本仅服务报表/页面原型设计、可运行原型、模板和原型验收；不承接技术方案、后端实现、前端正式接入或测试执行。用于识别、记录和维护需求、数据模型、API、后端、前端、权限、环境、性能、测试中的缺口和阻塞项。用户提到缺失字段、口径不清、数据源未知、样例缺失、筛选前数据完整性缺口、筛选数据粒度缺口、表关系不明、枚举/公式待定、鉴权头/环境变量缺少、性能限制未定、假设、待补充清单、gap ledger、阻塞开发或联调时触发。"
---
