...
description: "[原型阶段] 本阶段版本仅服务报表/页面原型设计、可运行原型、模板和原型验收；不承接技术方案、后端实现、前端正式接入或测试执行。用于执行跨阶段质量门禁和一致性检查。用户提到质量检查、开工前理解门禁、行动前自省、反懒性执行门禁、证据不足不能ready、过程中反思做得对不对、输入材料冲突、入口一致性、设计合理性、筛选绑定断裂、快照接口依赖、snapshotDate/dataVersion/loadBatch、指标单位/百分比显示冲突、更换数据源/数据表后API字段漂移、新增字段命名不规范、生产就绪、验收门禁、可交付性、不要凭空假设、需求/API/模型/前端/后端/测试不一致、ready/partial/blocked 判定时触发；不替代具体实现 skill。"
---
