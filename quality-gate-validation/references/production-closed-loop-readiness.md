...
| --- | --- | --- |
| Requirement and technical architecture | `report-requirement-structure-extraction`, `report-delivery-pipeline-governance`, `gap-ledger-management`, `delivery-artifact-template-management` | API/model artifacts may be traceable on paper but miss business scope, architecture boundaries, technology choices, implementation route, source authority, permissions, runtime/security, or model gaps. |
