...
For definition/help cards that explain metric meaning, formula,口径, denominator, conditions, source, or examples, also load `03b-definition-help-card-patterns.md`. These cards use `analysisPerspective: definitionHelp` and must select `definitionHelpCardPattern` before visual styling.
...
- Definition/metric explanation card: select `definitionHelpCardPattern`; show or disclose metric meaning, formula,口径, source/freshness, denominator, condition, and example evidence; complex definitions open drawer/popover.
