...
description: "[原型阶段] 独立生成报表、驾驶舱、管理看板、分析页面或数据应用的完整 PRD 文档，并作为所有报表原型设计 workflow 的前置输入契约。用户提到 PRD、需求文档、产品需求、需求转开发、报表需求、驾驶舱需求、原型前置需求、页面内容、页面布局、模板配置、指标清单、指标挂载矩阵、数据 API、交互逻辑、角色权限、一期范围边界时触发。输出必须覆盖背景目标、角色场景、范围边界、页面内容、页面布局配置、指标口径、指标挂载矩阵、数据/API、交互逻辑、验收缺口和 PRD-to-workflow 执行矩阵；不直接开发代码、不生成接口实现、不替代下游原型/技术方案/测试执行。"
---
...
2. Extract requirement evidence.
   Capture confirmed facts, inferred assumptions, missing gaps, source materials, target users, business lines, metric families, existing templates, and any explicit exclusions.
