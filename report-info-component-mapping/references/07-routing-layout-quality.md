...
- For top-level page blocks, `M * N` means page-grid column span by page-grid row span, and every visible parent block must use `N >= 3`. Any source/sample area smaller than 3 rows must be represented as internal component/sub-block sizing inside a legal parent block, merged with adjacent related content, or marked as a blocker.
...
- Text summary components use `text-summary`. Former one/two-row sizes such as `4*1`, `5*1`, `6*1`, `7*1`, `8*1`, or `3*2` are internal component/sub-block sizing hints only and must not be emitted as top-level `pageLayoutConfig` block spans. A standalone top-level summary block starts at `M*3`, such as `6*3` or `12*3`; smaller summary bars, insight cards, side panels, or annotation bubbles must live inside a legal parent block. When the text summary is an analysis/explanation/decision-support component, it must carry `analysisInsightContract`; recommended internal sizes are summary bar `36-56px`, small insight card `220-360px * 88-128px`, standard card `320-560px * 120-180px`, enhanced diagnosis card `480-720px * 160-240px`, side insight panel `220-320px * 240-480px`, and annotation bubble `120-240px * 40-96px`.
...
- The mapping can be implemented within the `12 * N` rectangular parent grid, every visible top-level parent block has `rowSpan >= 3`, and any internal sub-blocks have explicit component owner, local size, `5px` parent inset, `5px` sibling gap, state, and overflow rules.
