...
- At `1920x1080`, every visible item/card content viewport must pass `scrollHeight <= clientHeight + 2` and `scrollWidth <= clientWidth + 2`. Use DOM checks; screenshots are supporting evidence only. `scrollHeight > clientHeight + 2` or `scrollWidth > clientWidth + 2` is clipping.
