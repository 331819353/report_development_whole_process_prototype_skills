...
- The minimum top-level block row span is `N=3`; the default analytical/chart block is `3*3`, and any `N < 3` source/sample area is internal component/sub-block sizing only.
...
- Sankey blocks use at least a `4*3` top-level span, preserve `sankeyAreaH >= CH * 0.55`, declare source-target-value links and layer order, aggregate long-tail nodes/links with Top N/`其他`, keep labels key-only, and collapse legend/filters before compressing the flow body.
