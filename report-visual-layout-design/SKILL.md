...
| Latest zero-to-one configurable flow | `report-prototype-template-management` `references/configurable-zero-to-one-flow.md` when the layout will feed a configurable template project |
...
7. Place navigation, page/global filters, toolbar actions, brand/logo, refresh/export/fullscreen/status controls, block-level pill controls, and template-native filter entry when applicable. Every visible control must state owner, affected blocks/components, changed params, and refresh/state response.
...
10. Lay out top-level parent blocks in legal spans on the content-area grid: 12 equal columns after menu/sidebar width is deducted, and rowHeight from 8 visible row units after menu/header height is deducted. Row count is `N` and is not capped by the grid rule. In top-level `M*N` block notation, `N` is the page-grid row span and must be at least `3`; smaller source/sample areas become internal component/sub-block sizing inside a legal parent block or a blocker. Then decide whether each block contains one component or composed internal sub-blocks.
...
12. When the layout feeds a configurable template implementation, consume the PRD section 4C Markdown/mermaid page previews first, then output `pageLayoutConfig` and Template Build Packet layout fragments for `report-prototype-template-management`: packet sections 3 page registry, 4 page layout rows, 5 block map, and the layout-facing part of 6 standard block areas. Include `readerPreview` consumption proof, `layoutRows`, stable block ids, one-to-one `blockAreaConfigMap` handoff rows, block spans, nav/page wiring, first-viewport plan, and the intended `3 componentArea` slot count per block. Do not fill component slots here; component filling belongs to `componentSlotConfigMap` and `componentExampleConfigMap`.
...
- Header/navigation/filter/toolbar/brand structure, block-level pill placement, and visible interaction entry points.
...
- `pageLayoutConfig` handoff when template-based: PRD section 4C `readerPreview` consumption proof, Template Build Packet layout fragments, `layoutRows`, stable block ids, block spans, nav/page widget wiring, first-viewport plan, one-to-one `blockAreaConfigMap` rows to be created with `createBlockAreaConfig`, intended component-area slot count per block, SingleSlot/MultiSlot rationale, and first-slot conclusion placement for multi-slot blocks when applicable.
...
- Proof obligations for implementation-ready layout: legal grid spans, exact 12-column rows, minimum 8 rows, filter/control ownership, block pill placement, fixed-height overflow checks, first-viewport answer, component handoff, screenshot/DOM evidence, and exception conditions for advisory layout defaults.
...
- Do not mark a template-based layout implementation-ready unless `pageLayoutConfig` can be handed to `report-prototype-template-management` without guesswork: block ids are stable, spans are legal, nav/page wiring is named, every block can become one `blockAreaConfigMap` row, and each block's component-area slot need is explicit.
...
- Do not mark layout ready when filter surfaces, pill controls, toolbar actions, or drilldown/jump/modal/drawer/popup entry points that affect the report are missing visible placement and owner.
