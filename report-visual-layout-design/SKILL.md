...
- `pageLayoutConfig` handoff when template-based: `layoutRows`, stable block ids, block spans, nav/page widget wiring, first-viewport plan, intended component-area slot count per block, SingleSlot/MultiSlot rationale, and first-slot conclusion placement for multi-slot blocks when applicable.
