...
description: "[原型阶段] 本阶段版本仅服务报表/页面原型设计、可运行原型、模板和原型验收；不承接技术方案、后端实现、前端正式接入或测试执行。用于报表页面区块、容器、卡片、图表和表格的尺寸约束、高度预算、首屏层级、8*N网格、固定高度、overflow/裁切/重叠、响应式尺寸评审。用户提到卡片高度、图表太挤、表格太矮，KPI/趋势/对比/排名/占比/拆解/分布/地图/矩阵/VS/多组件挤压，1920x1080、1280x768、scrollHeight/clientHeight、plot压缩时触发；不负责组件内部图表细节。"
---
