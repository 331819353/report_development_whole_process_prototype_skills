...
PRD prerequisite: this workflow consumes a complete PRD bundle from `report-prd-document-generation`. The main PRD is the human/business authority; `CHILD-PRD-PROTOTYPE` is the AI-executable prototype contract. If the request has no PRD, or the PRD lacks the required PRD-to-workflow execution matrix, child PRD registry, or synced `CHILD-PRD-PROTOTYPE`, generate/update the PRD first and then continue from that contract. Do not recreate full PRD content inside this workflow.
...
- Child PRD bundle contract: `report-prd-document-generation` `references/child-prd-bundle-contract.md`; use it to validate section 0A child registry, section 14 child PRD bundle, final stage map, and `CHILD-PRD-PROTOTYPE` sync status before prototype work.
...
3. Load `report-prd-document-generation` `references/prototype-workflow-execution-map.md` and create/validate the PRD section execution matrix. Every PRD section, including section 4B management satisfaction gates when required, must map to a downstream owner capability, execution artifact, blocking rule, and status before workflow design starts.
3A. Validate the PRD readability contract from `report-prd-document-generation` `references/readable-prd-main-body.md`: the main PRD must be short and business-readable, every retained navigation page must have a Markdown/mermaid preview in section 4C, and dense IDs/matrices must live in execution appendices. If missing, update the PRD before layout or template work.
3B. Validate the child PRD bundle from `report-prd-document-generation` `references/child-prd-bundle-contract.md`: section 0A must explain the child PRD purpose and sync rule for 原型、前端、后端、技术方案、测试; section 14 must include `CHILD-PRD-PROTOTYPE`; and `CHILD-PRD-PROTOTYPE` must be `synced` or `draft` with non-blocking gaps before prototype design/layout/template work starts.
3C. For any template-based runnable prototype, create or validate the Template Build Packet from `report-prototype-template-management` `references/template-build-packet-contract.md`. Treat the packet as the single construction plan before source edits; if the PRD has only a seed, turn it into a current packet and mark rows ready/draft/blocked.
...
15B. Before bundled-template source edits, consume the Template Build Packet row by row. Do not edit a page, block, slot, filter/action, interaction, data/API source, conclusion rule, self-developed item, or target file that is absent from the packet.
15. Use `report-visual-layout-design` for shell, navigation, filter surface, `12 * N` grid, block sizing, page规范, and per-parent-block `blockChromePattern` selection. Its handoff must include `pageLayoutConfig`: `layoutRows`, stable block ids, readable `layoutCoordinateMap`, block spans, PRD `PATH-*` source per visible block, section 4B gate ID traces when applicable, first-viewport plan, and nav/page wiring. Use `blockCoordinate` as `R-B` and `slotCoordinate` as `R-B-S`, for example `1-2-1` = first slot inside the second block of page row 1.
16. Use `report-prototype-template-management` to execute the nine-step template operation flow and Template Build Packet: `frameworkTemplateId -> pageLayoutConfig -> blockLayoutTemplateMap -> titleAreaConfig -> pillAreaConfig -> auxMetricAreaConfig -> unitAreaConfig -> componentContentAreaTemplateMap -> summaryAreaConfig`. Select the independent 分块布局模板 Vue file for every page block, keep `blockCoordinate` in every block map row, configure title, decide pill buttons, configure evenly distributed additional information, decide units, then fill only `3 componentArea` slots with 组件内容区模板 and `slotCoordinate`, and configure summary/explanation on `4 summaryArea`. Every source edit must consume packet rows; any conclusion in `4 summaryArea` or a conclusion component must consume `conclusionRuleMap` instead of fixed copy.
...
- Child PRD proof: section 0A child registry exists, section 14 child PRD bundle exists, final stage map names child PRDs for 原型、前端、后端、技术方案、测试, `CHILD-PRD-PROTOTYPE` sync status, parent version consumed, and any stale/downstream child `GAP-*` impact.
- PRD readability proof: reader-facing main document exists, section 4C Markdown/mermaid preview exists for every retained navigation page, and raw IDs/matrices are kept in execution appendices rather than dominating the main body.
...
- Template operation chain: Template Build Packet path/status and packet-row consumption evidence, `frameworkTemplateId`, `pageLayoutConfig` plus `layoutCoordinateMap`, `blockLayoutTemplateMap` with `blockCoordinate` and selected independent block layout Vue files, `titleAreaConfig`, `pillAreaConfig`, `auxMetricAreaConfig`, `unitAreaConfig`, `componentContentAreaTemplateMap` with `slotCoordinate`, `summaryAreaConfig`, `conclusionRuleMap` consumption evidence, ECharts self-developed component content area fallbacks, and validation evidence for every filled slot.
...
- Do not start design, layout, component mapping, template copying, implementation, or repair when the PRD bundle lacks section 0A child registry, section 14 child PRD bundle, final stage map, or a usable `CHILD-PRD-PROTOTYPE`.
...
- Do not start layout, component mapping, or template copying when the PRD lacks section 4C Markdown/mermaid previews for every retained navigation page.
