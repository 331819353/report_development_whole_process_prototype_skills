...
Agent-neutral rule: this workflow is a capability contract, not a platform-specific instruction file. Capability IDs such as `report-prd-document-generation` and `report-prototype-template-management` are stable routing names; Claude, Hermes, Codex, or another agent must map them to equivalent local prompts, tools, workflow nodes, or documentation before execution.

## Positioning

Use this workflow only when the user asks for a report/page/dashboard prototype, runnable demo, screenshot/HTML restoration, template-based prototype, or preview URL. It orchestrates atomic capabilities and keeps implementation details in the owning capabilities.

PRD prerequisite: this workflow consumes a complete PRD bundle from `report-prd-document-generation`. The concise main PRD is the human/business authority; `CHILD-PRD-PROTOTYPE` plus execution appendices are the AI-executable prototype contract. If the request has no PRD, or the PRD lacks the child PRD registry, execution appendices/matrix, or usable `CHILD-PRD-PROTOTYPE`, generate/update the PRD first and then continue from that contract. Do not recreate full PRD content inside this workflow.
...
- Main PRD minimal contract: `report-prd-document-generation` `references/main-prd-minimal-contract.md`; use it to validate that the main PRD is a concise decision brief and not the execution manual.
- Readable PRD contract: `report-prd-document-generation` `references/readable-prd-main-body.md`; use it to validate that the PRD has a short readable main document, Markdown navigation/page previews, and execution appendices instead of one long ID-heavy body.
- Child PRD bundle contract: `report-prd-document-generation` `references/child-prd-bundle-contract.md`; use it to validate the child registry, full child PRD bodies or files, final stage map, and `CHILD-PRD-PROTOTYPE` sync status before prototype work.
...
- Template step operation cards: `report-prototype-template-management` `references/template-step-operation-cards.md` before executing or accepting each step in the nine-step template operation flow.
- Template asset construction understanding: `report-prototype-template-management` `references/template-asset-construction-contract.md` before selecting block layout templates or component content area templates.
- Template Build Packet contract: `report-prototype-template-management` `references/template-build-packet-contract.md` before PRD-driven template implementation, source edits, or readiness acceptance.
...
1. Run the Preflight understanding gate before layout, template, styling, or code. Name the workflow mode, evidence inventory, authority order, affected surfaces, owning capabilities, hard constraints, missing evidence, and start decision.
...
3. Load `report-prd-document-generation` `references/prototype-workflow-execution-map.md` and create/validate the PRD execution matrix. Every main PRD section, execution appendix, and child PRD needed by prototype work must map to a downstream owner capability, execution artifact, blocking rule, and status before workflow design starts.
3A. Validate the PRD readability contract from `report-prd-document-generation` `references/main-prd-minimal-contract.md` and `references/readable-prd-main-body.md`: the main PRD must be short and business-readable, every retained navigation page must have a Markdown/mermaid preview, and dense IDs/matrices must live in execution appendices or child PRDs. If missing, update the PRD before layout or template work.
3B. Validate the child PRD bundle from `report-prd-document-generation` `references/child-prd-bundle-contract.md`: the main PRD child registry must explain purpose and sync rules for 原型、前端、后端、技术方案、测试; the bundle must include a usable `CHILD-PRD-PROTOTYPE`; and `CHILD-PRD-PROTOTYPE` must be `synced` or `draft` with non-blocking gaps before prototype design/layout/template work starts.
3C. For any template-based runnable prototype, create or validate `templateAssetUnderstandingMap` from `report-prototype-template-management` `references/template-asset-construction-contract.md`, then create or validate the Template Build Packet from `report-prototype-template-management` `references/template-build-packet-contract.md`. Treat the packet as the single construction plan before source edits; if the PRD has only a seed, turn it into a current packet and mark rows ready/draft/blocked.
...
8. Load `report-prototype-design-thinking` as the default generic thinking layer. Do not branch into 自助分析、指标看板、分析报告、or 明细报表 inside this workflow; those are separate workflow capabilities.
...
10. Use `report-info-component-mapping` to produce analysis perspectives, answer atoms, component bundles, datasets, filters, interactions, binding matrix, and a filter/value semantics table from `CHILD-PRD-PROTOTYPE` report-path rows and executive gate rows when applicable.
...
15A. Before bundled-template execution, load `report-prototype-template-management` `references/template-step-operation-cards.md`. During the nine-step template flow, each step must state its current focus, read only its step inputs, output the step result, run the stop check, and avoid later-step work until the current step is complete or blocked.
15B. Before bundled-template source edits, consume `templateAssetUnderstandingMap` and the Template Build Packet row by row. Do not edit a page, block, slot, filter/action, interaction, data/API source, conclusion rule, self-developed item, or target file that is absent from the packet.
15. Use `report-visual-layout-design` for shell, navigation, filter surface, `12 * N` grid, block sizing, page规范, and per-parent-block `blockChromePattern` selection. Its handoff must include `pageLayoutConfig`: `layoutSectionMap`, `layoutRows`, stable block ids, readable `layoutCoordinateMap`, block spans, `CHILD-PRD-PROTOTYPE` path source per visible block, executive gate ID traces when applicable, first-viewport plan, and nav/page wiring. Use `blockCoordinate` as `R-B` and `slotCoordinate` as `R-B-S`, for example `2-2-1` = first slot inside the second block of section 2.
16. Use `report-prototype-template-management` to execute the nine-step template operation flow and Template Build Packet: `frameworkTemplateId -> templateAssetUnderstandingMap -> pageLayoutConfig -> blockLayoutTemplateMap -> titleAreaConfig -> pillAreaConfig -> auxMetricAreaConfig -> unitAreaConfig -> componentContentAreaTemplateMap -> summaryAreaConfig`. Select the independent direct 分块布局模板 Vue file for every page block, never a size-only `SpanCCxRRLayout` wrapper, keep `blockCoordinate`, `slotCount`, `componentSlotPattern`, and `slotCoordinateList` in every block map row, configure title, decide pill buttons, configure evenly distributed additional information, decide units, then fill only declared `3 componentArea` slots with 组件内容区模板, `slotCoordinate`, slot pattern code, and visual-type size compatibility evidence, and configure summary/explanation on `4 summaryArea`. Every source edit must consume packet rows; any conclusion in `4 summaryArea` or a conclusion component must consume `conclusionRuleMap` instead of fixed copy.
...
- Workflow mode, Preflight understanding matrix, input inventory, prototype design-thinking output, `CHILD-PRD-PROTOTYPE` report-type implementation path and executive satisfaction gate consumption when applicable, core narrative, user path, key decision points, target user/scenario/decision/action, `displayTheme`, pattern cards, report type, and core question.
- PRD prerequisite proof: PRD status, source PRD path or generated output, PRD-to-workflow execution matrix, blocked/draft rows, and how every main PRD section, execution appendix, and child PRD is consumed or explicitly deferred.
- Child PRD proof: main PRD child registry exists, full child PRD bundle or child files exist, final stage map names child PRDs for 原型、前端、后端、技术方案、测试, `CHILD-PRD-PROTOTYPE` sync status, parent version consumed, and any stale/downstream child `GAP-*` impact.
- PRD readability proof: concise reader-facing main document exists, Markdown/mermaid preview exists for every retained navigation page, and raw IDs/matrices are kept in execution appendices/child PRDs rather than dominating the main body.
- Template asset understanding proof: `templateAssetUnderstandingMap` with selected asset root, shell config source, page-layout grid contract, block layout library evidence, direct selectable block template availability, component content area library evidence, widget schema source, validator source, and unresolved asset gaps.
- Template Build Packet proof: packet path/status, PRD seed consumed, current packet sections ready/draft/blocked, and packet rows mapped to implementation target files.
...
- Affected-surface to owning-capability routing, including chart/table/filter/component-placement/design-system capabilities when applicable.
...
- Layout plan, selected framework template, template-only decision, self-development exception map, filter surface mapping, `CHILD-PRD-PROTOTYPE` path and gate IDs to block mapping, perspective-layer mapping, baseline inheritance decision, anti-squeeze row-group/vacancy reflow decisions when triggered, and implementation target path.
- Template operation chain: `templateAssetUnderstandingMap`, Template Build Packet path/status and packet-row consumption evidence, `frameworkTemplateId`, `pageLayoutConfig` plus `layoutSectionMap` and `layoutCoordinateMap`, `blockLayoutTemplateMap` with `blockCoordinate`, `slotCount`, `componentSlotPattern`, `slotCoordinateList`, selected direct independent block layout Vue files, and asset availability, `titleAreaConfig`, `pillAreaConfig`, `auxMetricAreaConfig`, `unitAreaConfig`, `componentContentAreaTemplateMap` with `slotCoordinate`, slot pattern code, component slot size, visual-type size compatibility evidence, `summaryAreaConfig`, `conclusionRuleMap` consumption evidence, ECharts self-developed component content area fallbacks, and validation evidence for every filled slot.
- Step operation checkpoints for the nine-step template flow: current step, inputs read, operation result, stop check, and blocker/gap for steps 1 through 9.
- Packet-row consumption evidence for template implementation: each edited target file maps to packet section 11 and each implemented page/block/slot/control/data/conclusion row has validation evidence.
...
- Do not start design, layout, component mapping, template copying, implementation, or repair when the PRD bundle lacks a main child registry, full child PRD bundle or child files, final stage map, or a usable `CHILD-PRD-PROTOTYPE`.
- Do not start layout, component mapping, or template copying when `CHILD-PRD-PROTOTYPE` lacks report path rows, first-viewport plan, and path-step-to-block mapping.
- Do not start layout, component mapping, or template copying for management-facing reports when `CHILD-PRD-PROTOTYPE` lacks required executive satisfaction, severity, action, trust, or review/export rows.
- Do not start layout, component mapping, or template copying when the main PRD lacks Markdown/mermaid previews for every retained navigation page.
- Do not start template source edits when the Template Build Packet is missing, blocked, stale, or lacks rows for the page/block/slot/control/data/conclusion/target file being implemented.
- Do not start layout, template selection, component slot filling, or source edits when `templateAssetUnderstandingMap` is missing, stale, or not based on the selected template asset root.
...
- Do not rely only on this top-level workflow when a chart, table, filter, component-placement, or reusable component standard is affected; route to the specific front-door capability before implementation or acceptance.
...
- Do not start bundled-template implementation or accept it as ready when step operation checkpoints from `report-prototype-template-management` `references/template-step-operation-cards.md` are missing for any step from 1 through 9.
- Do not accept bundled-template implementation when edited source files cannot be traced back to Template Build Packet section 11 and the corresponding packet rows.
- Do not implement or accept a bundled-template prototype before the nine-step template operation chain exists: `frameworkTemplateId`, `templateAssetUnderstandingMap`, `pageLayoutConfig`, `blockLayoutTemplateMap`, `titleAreaConfig`, `pillAreaConfig`, `auxMetricAreaConfig`, `unitAreaConfig`, `componentContentAreaTemplateMap`, and `summaryAreaConfig`. `blockLayoutTemplateMap` must name the selected direct independent block layout Vue file, not a size-only `SpanCCxRRLayout` wrapper and not only `componentRegionPattern`. Every `3 componentArea` slot must be filled by an existing 组件内容区模板 or a newly registered standalone ECharts component content area template with visual-type size compatibility evidence.
- Do not implement or accept a bundled-template prototype when any visible block lacks `blockCoordinate`, any component slot lacks `slotCoordinate`, or coordinates are inconsistent with the PRD/Template Build Packet, `layoutRows`, block spans, selected block layout slot order, metric mounting, conclusion rules, or interactions.
- Do not implement or accept a bundled-template prototype when `layoutSectionMap` is missing, section row counts do not sum to page `N`, any section is not exact `12*K`, any block lacks `slotCount` / `componentSlotPattern` / `slotCoordinateList`, or any component template maps to an undeclared slot.
