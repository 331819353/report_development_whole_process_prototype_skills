...
- `pageLayoutConfig` handoff for template implementations: `templateAssetUnderstandingMap`, `layoutSectionMap` with exact `12*K` readable sections, `layoutRows`, stable block ids, readable `layoutCoordinateMap`, block spans, nav/page wiring, first-viewport plan, intended component-area `slotCount`, `componentSlotPattern`, and `slotCoordinateList` per block, and Template Build Packet layout fragments for sections 2A, 3, 3A, 4, 5, and layout-facing section 6.
...
- Template implementations must follow `report-prototype-template-management` `references/template-operation-flow.md`, `references/template-step-operation-cards.md`, `references/template-asset-construction-contract.md`, and `references/template-build-packet-contract.md`: `frameworkTemplateId -> templateAssetUnderstandingMap -> pageLayoutConfig -> blockLayoutTemplateMap -> titleAreaConfig -> pillAreaConfig -> auxMetricAreaConfig -> unitAreaConfig -> componentContentAreaTemplateMap -> summaryAreaConfig`. `pageLayoutConfig` must carry `layoutSectionMap` before raw `layoutRows`; `blockLayoutTemplateMap` must name selected direct independent block layout Vue files, not size-only `SpanCCxRRLayout` wrappers, and declare `slotCount`, `componentSlotPattern`, and `slotCoordinateList`; `componentContentAreaTemplateMap` must map each declared `slotCoordinate` to a registered component content area template, sample/source evidence, and visual-type size compatibility evidence. Custom ECharts chart work must become a standalone 组件内容区模板 before it fills a `3 componentArea` slot. Source edits must consume Template Build Packet rows rather than scattered PRD prose.
...
- Implement block slots through independent 分块布局模板 Vue files and registered 组件内容区模板. `componentSlots` under `3 componentArea` must carry a registered `componentContentAreaTemplateId`, standalone Vue file, copy source/target, sample evidence, props/data/state contract, and data binding; they must not carry title, pill, additional information, unit, summary, explanation content, text placeholders, or visualType-only fills. Those supporting areas stay on block template areas `1-1`, `1-2`, `2-1`, `2-2`, and `4`.
- Template controls must be visibly placed before runtime acceptance: `filterSurfaceMap`, `toolbarActionMap`, `pillAreaConfig`, and `interactionBehaviorMap` must cover page filters, business switches, date/period controls, block pills, drilldown, jump, modal, drawer, popup, refresh, export, and fullscreen behavior when present.
