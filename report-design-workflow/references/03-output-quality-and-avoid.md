...
- Dense or metric-bearing blocks carry `layoutFitContract` before layout and implementation. If one core block in a semantic row group needs more height, the row group expands/reflows together. If a block moves to a wider or full-row span, the old vacancy is handled by related-content fill, value-adding sibling expansion, row-group repack, or a named blocker. Font shrinkage, hidden overflow, empty sibling stretching, fake placeholders, and low-priority filler fail readiness.
