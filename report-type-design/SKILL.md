...
description: "[原型阶段] 本阶段版本仅服务报表/页面原型设计、可运行原型、模板和原型验收；不承接技术方案、后端实现、前端正式接入或测试执行。用于选择和设计报表业务类型，独立输出报表类型、核心问题、指标/维度、组件建议、筛选交互、证据链和验收口径。用户提到状态总览、驾驶舱、分析诊断、原因归因、明细查询、台账、绩效考核、评分排名、汇报复盘、异常监控、预警告警、任务执行、待办闭环、核对对账、差异追溯、审计证据时触发；不负责页面模板工程、API文档、后端实现或运行测试。"
---
