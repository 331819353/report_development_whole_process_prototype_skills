...
- PRD prerequisite proof: PRD status, section 0A child registry, section 14 child PRD bundle, `CHILD-PRD-PROTOTYPE` sync status, section 4A `RTP-*` / `PATH-*` rows consumed, section 13 Template Build Packet seed/current packet proof, PRD-to-workflow execution matrix, detail-report rows consumed, blocked/draft rows, and deferred-out-of-scope rows.
...
- Template operation chain: Template Build Packet path/status and packet-row consumption evidence, `frameworkTemplateId`, `pageLayoutConfig` plus `layoutCoordinateMap`, `blockLayoutTemplateMap` with `blockCoordinate` and selected independent block layout Vue files, `titleAreaConfig`, `pillAreaConfig`, `auxMetricAreaConfig`, `unitAreaConfig`, `componentContentAreaTemplateMap` with `slotCoordinate`, `summaryAreaConfig`, `conclusionRuleMap` consumption evidence when generated conclusions exist, and self-developed component content area fallback list.
...
- Do not start query path, table field design, export/traceability, layout, template, or implementation work without a PRD bundle from `report-prd-document-generation` that includes section 0A child registry, section 14 child PRD bundle, and usable `CHILD-PRD-PROTOTYPE`.
