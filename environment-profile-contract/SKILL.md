...
description: "[原型阶段] 本阶段版本仅服务报表/页面原型设计、可运行原型、模板和原型验收；不承接技术方案、后端实现、前端正式接入或测试执行。用于定义或审计跨前端、后端、测试、生产的环境Profile契约。用户提到环境Profile、.env.test、.env.production、测试生产隔离、API base URL与后端Profile对应、proxy/CORS/base path、生产不能指向mock/local/test、环境交付验收、配置契约、运行URL和Profile一致性时触发；不替代单个前端环境修复或URL冒烟。"
---
