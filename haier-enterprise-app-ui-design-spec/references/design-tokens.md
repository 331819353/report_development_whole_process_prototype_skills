...
- 报表/仪表盘/BI 页面继承本色彩体系时，应先建立品牌/产品色体系：品牌主色、延展色、中性色、页面层级色、选中/聚焦/主行动色和主数据强调色。功能语义色不是页面默认语言，不能用红/绿替代品牌色体系来组织信息。
...
- 功能色可用于图标、组件、弹层等场景。
- 功能语义包括链接、成功、警告、错误。
- 在报表、仪表盘、BI 和数据分析页面中，绿色/红色/金色只用于已定义的成功、警告、错误、风险、健康、异常、涨跌或业务方向语义；普通层级、分类、强调、导航、选中和主数据焦点优先使用品牌/产品色与中性色。语义色必须配合文本、符号、图标、阈值或说明，不应单独承担含义。
