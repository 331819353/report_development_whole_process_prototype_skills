...
- 配置化项目决策: select `frameworkTemplateId`, target path, `pageLayoutConfig`, `blockAreaConfigMap`, and `componentExampleConfigMap`; configure the selected bundled template project and preserve Vue 3 + TypeScript + Vite + Element Plus + ECharts + axios. Unsupported shell/layout requirements become blockers or template backlog items.
