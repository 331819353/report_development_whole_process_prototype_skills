...
- 交付形态决策: default prototype output is `vueTemplatePrototype` with Vue 3 + TypeScript + Vite + Element Plus + ECharts + axios; choose `htmlPrototype` only when the user explicitly asks for HTML/static/single-file HTML output or exact static HTML preservation.
