...
- If the user provides HTML/MD/source files for a report/page prototype without explicitly asking for HTML/static output, still route as `prototype-design` and hand off to the default `copyTemplateProject` path: copy a bundled template project and preserve Vue 3 + TypeScript + Vite + Element Plus + ECharts + axios.
