...
description: "[原型阶段] 本阶段版本仅服务报表/页面原型设计、可运行原型、模板和原型验收；不承接技术方案、后端实现、前端正式接入或测试执行。用于治理报表项目从需求、原型、数据模型、API、后端、前端、测试到发布/复测的交付链路、阶段交接和ready/partial/blocked判定。用户提到交付链路、阶段门禁、上下游交接、需求到原型/API/前后端/测试是否闭合、artifact contract、handoff、哪个阶段该走哪个skill、跨阶段缺口时触发；不替代单个阶段的具体实现。"
---
