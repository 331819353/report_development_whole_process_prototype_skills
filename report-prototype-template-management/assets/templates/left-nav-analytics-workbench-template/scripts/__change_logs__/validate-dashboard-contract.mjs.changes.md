...
- Affected contracts: 1920x1080 prototype content grid; vertical menu width deduction; 12 fixed columns; `N` uncapped rows; 8 visible row units for rowHeight.
...
- Affected contracts: Bundled left-nav template layoutRows, widget visualType span validation, prototype 1920x1080 grid rules.
- Verification: `npm run validate:dashboard` passed in `left-nav-analytics-workbench-template`.
