...
const fullscreenOffsetX = ref(0);
const fullscreenOffsetY = ref(0);
...
const contentWidth = computed(() =>
  Math.max(props.config.screen.layout.designWidth - props.config.screen.layout.sidebarWidth, 1),
);
...
const appStyle = computed(() => ({
  ...
  '--fullscreen-scale': String(fullscreenScale.value),
  '--fullscreen-offset-x': `${fullscreenOffsetX.value}px`,
  '--fullscreen-offset-y': `${fullscreenOffsetY.value}px`,
...
const getBlockContentWidth = (block: LayoutBlock) => {
  const contentWidth = Math.max(props.config.screen.layout.designWidth - props.config.screen.layout.sidebarWidth, 1);
  const columnWidth = contentWidth / Math.max(layoutColumnCount.value, 1);
...
const setFullscreenDocumentState = (active: boolean) => {
  document.documentElement.classList.toggle('dashboard-fullscreen-active', active);
  document.body.classList.toggle('dashboard-fullscreen-active', active);
};
...
const updateViewportLayout = () => {
  ...
  fullscreenOffsetX.value = 0;
  fullscreenOffsetY.value = 0;
  pageScrollX.value = 0;
};
...
const updateFullscreenLayout = () => {
  ...
  isDashboardFullscreen.value = active;
  setFullscreenDocumentState(active);
  updateViewportLayout();
  pageScrollX.value = 0;
};

const scheduleFullscreenLayoutUpdate = () => {
  window.requestAnimationFrame(updateFullscreenLayout);
};

const toggleFullscreen = async () => {
  if (document.fullscreenElement) {
    await document.exitFullscreen();
    scheduleFullscreenLayoutUpdate();
    return;
  }
...
onBeforeUnmount(() => {
  ...
  window.clearTimeout(scrollbarsHideTimer);
  setFullscreenDocumentState(false);
});
...
<template>
  ...
  <main
    ...
    :class="[
      `theme-${theme}`,
      {
        'sidebar-collapsed': !isNavOpen,
        'filters-open': isFiltersOpen,
        'dashboard-fullscreen': isDashboardFullscreen,
      },
    ]"
    :style="appStyle"
  >
              ...
              >
                ...
                >
                  <p class="placeholder-cell-summary-text">
                    {{ getWidgetBodySummary(block.label) }}
                  </p>
                </section>
              </div>
              <div
