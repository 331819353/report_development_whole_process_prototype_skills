...
This directory is the block layout template library for the topbar light scroll dashboard template.

Terminology used by the report development flow:

- 框架模板: the page shell, such as top navigation, filter entry, toolbar, theme, and runtime stack.
...
- Block background: one shared light surface, covering the full block including the top zone and body zone
