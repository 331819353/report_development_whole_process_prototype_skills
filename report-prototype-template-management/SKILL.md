...
15. Declare control ownership before adding UI controls. Default: template owns page/global filters, refresh, download/export, topbar actions, logo/title/navigation, and route-level toolbar. Page interactions should be configured with `actions` on the block widget or component slot first; shell defaults can execute route, external, drawer/drilldown, modal, popover/popup, cross-filter, fullscreen, export, and refresh. Use `src/actions/registry.ts` only for custom permission, API orchestration, analytics, host integration, or business-specific overlay behavior. Do not move shell-owned controls into components inside the report development flow.
