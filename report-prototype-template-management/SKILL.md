...
2. Design 页面布局配置 through `layoutRows`, stable block ids, PRD section 4A `PATH-*` source per visible block, PRD section 4B gate ID traces when applicable, and page/nav widget wiring; for templates with `nav[]`, define substantial nav-page content before copying or editing.
...
- Template operation chain: `frameworkTemplateId`, `pageLayoutConfig`, PRD `RTP-*` / `PATH-*` trace and applicable section 4B gate ID traces per visible block, `blockLayoutTemplateMap` with each block's selected independent block layout Vue file, `titleAreaConfig`, `pillAreaConfig`, `auxMetricAreaConfig`, `unitAreaConfig`, `componentContentAreaTemplateMap`, `summaryAreaConfig`, `conclusionRuleMap` consumption evidence, and `selfDevelopmentExceptionMap` entries when component content area templates or interaction behavior are self-developed.
