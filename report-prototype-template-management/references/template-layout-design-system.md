...
Required chain: `frameworkTemplateId -> pageLayoutConfig -> filterSurfaceMap -> toolbarActionMap -> interactionBehaviorMap -> blockLayoutTemplateMap -> titleAreaConfig -> pillAreaConfig -> auxMetricAreaConfig -> unitAreaConfig -> componentContentAreaTemplateMap -> summaryAreaConfig`. `pageLayoutConfig` must include `layoutCoordinateMap`; `blockLayoutTemplateMap` must name `blockCoordinate`, the independent block layout Vue file for every selectable block, and SingleSlot/MultiSlot rationale; `componentContentAreaTemplateMap` must name `slotCoordinate` for every component slot. Only `1-1 titleArea` and `3 componentArea` are always required; optional supporting areas can be omitted or marked not configured when not needed. Internal area codes such as `1-2 pillArea` are not page coordinates; standard area targets use `blockCoordinate + areaName`, such as `1-2:pillArea`.
