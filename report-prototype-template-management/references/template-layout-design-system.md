...
3. Based on the 页面布局配置, select the independent 分块布局模板 Vue file for each block and derive `slotCoordinate` (`R-B-S`) for every `3 componentArea` slot.
...
8. Based on the selected 分块布局模板 slot configuration, inspect `references/component-content-area-template-map.md`, the copied template's component-content-area catalog, and `src/report-template-assets/blueprint/widget-config-schemas.ts`; choose suitable 组件内容区模板 for `3 componentArea` only when the component slot size and visual type are compatible. When no suitable template exists, self-develop a new ECharts-backed standalone Vue component content area template.
