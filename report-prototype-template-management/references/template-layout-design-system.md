...
| 分块配置 | `createBlockAreaConfig`, `blockAreaConfigMap`, derived `componentRegionPattern`, `componentSlotContracts`, block area config | A block area config rendered by `BaseLayoutSpan.vue`. `layoutRows` determines size; the config owns only `1-1 titleArea`, `1-2 pillArea`, `3 componentArea`, and `4 summaryArea`. `componentRegionPattern` is only slot geometry. |
| 组件示例 | Standalone Vue file mapped by a registered `componentExampleId` | The implemented component's internal content area only. It can fill slots inside `3 componentArea`; do not include parent-block filters, controls, title pills, descriptions, explanations, or summary copy. It renders as a rounded rectangle without border lines, may reserve a removable `20px` centered title strip with `3px` top padding, and may render component-owned internal unit and auxiliary metrics such as `unit` and `auxMetrics` when declared by the example. Parent single-slot block layouts hide the title strip only. Text/prose or `componentSlots[].content` alone is not a valid slot fill. |
...
6. Based on the selected 分块配置 slot configuration, inspect `references/component-examples/config.ts`, the selected template's component example catalog, and `src/report-template-assets/blueprint/widget-config-schemas.ts`; choose suitable 组件示例 for `3 componentArea` only when the component slot size and visual type are compatible. Component-internal title, unit, auxiliary metrics, chart/table/list body, and overflow behavior must be provided by that registered component example. When no suitable example exists, self-develop a new ECharts-backed standalone Vue component example and register it before filling the slot.
...
Required chain: `frameworkTemplateId -> templateAssetUnderstandingMap -> pageLayoutConfig -> filterSurfaceMap -> toolbarActionMap -> interactionBehaviorMap -> blockAreaConfigMap -> titleAreaConfig -> pillAreaConfig -> componentExampleConfigMap -> summaryAreaConfig`. `pageLayoutConfig` must include `layoutCoordinateMap`; `blockAreaConfigMap` must name `blockCoordinate`, the block area config created by `createBlockAreaConfig` for every block, asset availability, and SingleSlot/MultiSlot rationale; `componentExampleConfigMap` must name `slotCoordinate`, component slot size, visual-type size compatibility evidence, and component-owned title/unit/auxMetrics config when declared by the selected example. Only `1-1 titleArea` and `3 componentArea` are always required; `1-2 pillArea` and `4 summaryArea` are optional block areas that can be omitted or marked not configured when not needed. Internal area codes such as `1-2 pillArea` are not page coordinates; standard area targets use `blockCoordinate + areaName`, such as `1-2:pillArea`.
...
8. Component content area boundary: when using the 分块配置 taxonomy, block-level title, pill controls, component slots, and summary content map only to `1-1 titleArea`, `1-2 pillArea`, `3 componentArea`, and `4 summaryArea`; they must not be duplicated inside component slots. The 组件示例 may have its own optional `20px` component example title strip for multi-slot disambiguation plus body content, including component-owned internal unit and auxiliary metric strips such as `unit` and `auxMetrics`; it must not render local filters, panel triggers, extra controls, parent-block titles/pills, descriptions, explanations, or summaries.
...
- Page layout and template shell do not render block-internal titles, local filters, local action links, or component-specific control strips. For the template-configuration flow, block titles, block pills, component slots, and summaries live in the 分块配置, while 组件示例 slot fills expose the optional removable title strip plus body and any declared component-owned internal unit or auxiliary metrics.
- For current 分块配置 implementations, visible block titles, pill controls, component slots, and summaries are owned by the 分块配置 block areas. A 组件示例 may use `widget.title`/props for its optional removable `20px` title strip and may use `unit`, `auxMetrics`, or similar props for internal unit and auxiliary metric strips.
...
- For 分块配置 usage, block-internal title text lives in `1-1 titleArea`, not inside the 组件示例. The 组件示例 may show its optional removable content-title strip for metric/content meaning and component-owned internal auxiliary metrics when declared.
