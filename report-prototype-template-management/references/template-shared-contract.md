...
- 分块配置: the block-level config created by `createBlockAreaConfig` and rendered by `BaseLayoutSpan.vue`. It owns `componentSlotContracts` and the standard areas `1-1 titleArea` 标题区, `1-2 pillArea` 胶囊按钮区, `2-1 auxMetricArea` 附加信息区, `2-2 unitArea` 单位区, `3 componentArea` 组件区, and `4 summaryArea` 说明区. `componentRegionPattern` is only an internal/compatibility descriptor derived from the selected template.
- 组件示例: the implemented component's internal content area only, packaged as an independently reusable Vue file. It may fill a component slot inside `3 componentArea`, but it must not carry filters, controls, additional information, unit slot, title pills, description/help text, or summary copy. It renders as a rounded rectangle without border lines and may expose only a removable `20px` centered top title strip; hide that strip when the parent 分块配置 has only one component slot.
...
8. Based on the selected 分块配置 slot configuration, inspect `references/component-examples/config.ts`, `src/widgets/templates/component-examples/` with its README/catalog, and `src/report-template-assets/blueprint/widget-config-schemas.ts`, then choose suitable registered 组件示例 for `3 componentArea`; every component example row maps one declared `slotCoordinate` plus slot pattern code, component slot size, and visual-type size compatibility evidence to a concrete template ID and sample/source evidence. If no suitable registered component example exists, self-develop a new standalone Vue component example with ECharts for chart needs and register it before the slot is treated as filled.
...
Required chain: `frameworkTemplateId -> templateAssetUnderstandingMap -> pageLayoutConfig -> filterSurfaceMap -> toolbarActionMap -> interactionBehaviorMap -> blockAreaConfigMap -> titleAreaConfig -> pillAreaConfig -> auxMetricAreaConfig -> unitAreaConfig -> componentExampleConfigMap -> summaryAreaConfig`. The chain materializes and configures the selected framework template project; unsupported shell/layout needs become blockers or template backlog items. `pageLayoutConfig` includes `layoutSectionMap`, an exact-12-column, over-12 rejection, and minimum-8-row `layoutRows` audit plus `layoutCoordinateMap`; `layoutSectionMap` splits pages into exact `12*K` parts and local A/B previews must be disambiguated in final machine rows. `pageLayoutConfig` and `blockAreaConfigMap` preserve PRD section 4A `RTP-*` / `PATH-*` source and applicable section 4B `ESG-*` / `SEV-*` / `ACT-*` / `TRUST-*` / `MEET-*` gate IDs for every visible block. `blockAreaConfigMap` records `blockCoordinate` (`R-B`), the block area config created by `createBlockAreaConfig`, asset availability, SingleSlot/MultiSlot rationale, `slotCount`, `componentSlotPattern`, and `slotCoordinateList` for every block. `componentExampleConfigMap` records `slotCoordinate` (`R-B-S`), slot pattern code, component slot size, and visual-type size compatibility evidence for every `3 componentArea` slot; for example `2-2-1` means the first component slot inside the second block of layout section/page region 2. Only `1-1 titleArea` and `3 componentArea` are always required; `1-2`, `2-1`, `2-2`, and `4` are optional and can be `null` with a reason. The internal area codes such as `1-2 pillArea` are not page coordinates; standard area targets use `blockCoordinate + areaName`, such as `2-2:pillArea`. When a component slot needs custom chart work, create `selfDevelopmentExceptionMap` entries with `type: componentExample` and register those standalone Vue files inside the configured project before treating the slots as filled.
...
- Block slot structure -> block area config created by `createBlockAreaConfig` plus parent block config (`componentSlotContracts`, optional derived `componentRegionPattern`, parent block `displayTitle`/title style, `titlePills`, `auxMetrics`, unit, `bodySummary`) using the standard areas `1-1`, `1-2`, `2-1`, `2-2`, `3`, and `4`. Every block map row must expose `slotCount`, `componentSlotPattern`, and `slotCoordinateList`. Do not put supporting areas inside `componentSlots`.
...
- Component-owned title/control areas are allowed only for ordinary business widgets outside the current 组件示例 slot contract. A 组件示例 inside `3 componentArea` may show only its optional removable title strip plus body content; `localFilters[]`, filter-panel triggers, and lightweight links belong to non-slot widgets, shell/page config, or an explicit template redesign.
