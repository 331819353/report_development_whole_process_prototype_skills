...
Use this file when materializing a configured template project, locating extension points, or deciding which files should own a change.
...
- `src/widgets/templates/block-spans/`: block area runtime and contracts, especially `BaseLayoutSpan.vue`, `types.ts`, catalog helpers, and slot-contract README.
- `src/widgets/templates/component-examples/`: standalone 组件示例 Vue files and component examples README.
...
- `src/actions/registry.ts`: optional shell-level/custom action hook registry. Configured interactions should start with block or component-slot `actions`; the shell has defaults for route, external, drawer/drilldown, modal, popover/popup, cross-filter, fullscreen, export, and refresh. Use the registry only to override defaults or integrate permission, API orchestration, analytics, host systems, or business-specific overlays.
- `src/styles.css`: template shell styles only.
- `public/`: logo and static assets.
- `scripts/validate-dashboard-contract.mjs`: dashboard contract validator.
- `scripts/init-code-ledgers.mjs`: project-wide baseline sidecar ledger initializer/checker for configured template projects.
- `scripts/update-code-ledger.mjs`: sidecar code change ledger helper for configured template projects.
...
1. Materialize `assets/templates/<template-id>/` into the target configured template project.
   This materialization is part of the supported configurable report route. Do not replace it with an independent scaffold inside this workflow; unsupported shell/layout requirements become blockers or template backlog items.
   Before the first source edit, run `npm run ledger:init` in the configured project to create baseline `__change_logs__` sidecars for all scoped source files. This is the project-level bootstrap; still use `npm run ledger:code -- --file <source-file> --stage before/after` around each edited file.
2. Keep the base stack dependencies as an integrated contract: `vue`, `@vitejs/plugin-vue`, `vite`, `typescript`, `vue-tsc`, `element-plus`, `echarts`, and `axios`. Do not remove ECharts or Element Plus because the first component batch is simple. Add `@antv/s2` and `@antv/s2-vue` only when the implementation contains pivot tables, cross tables, wide metric matrices, frozen-header analytical tables, or equivalent S2-class table needs.
   PRD/source mentions of HTML do not change this default stack; source material is mapped into the configured template project.
3. Before editing any configured template source file, create or update `docs/template-build-packet.md` for PRD-driven or multi-step work, then run or manually follow the code ledger trigger: `npm run ledger:code -- --file <source-file> --stage before`. Read the sidecar ledger before changing the file.
...
5. Edit `src/config/dashboard.config.ts` incrementally from the selected template baseline. Keep the selected template's native nav/page shape, filter trigger/panel/popover pattern, toolbar placement, theme fields, and shell-owned state. Map requirement-document title/filter/navigation/toolbar intent into these existing config slots instead of recreating the requirement shell; template-level redesign requests are blockers or template backlog items for report development.
6. Put mock data in `src/data/dashboard.dataset.json`, or configure `data.id: 'apiData'` / `source.id: 'apiData'` with an `api` block in `dashboard.config.ts`. When backend-shaped data is needed during prototype work, prefer the built-in npm mock API: `npm run mock:api` exposes `dashboard.dataset.json`, and `npm run dev:mock` starts it with Vite proxy. In mock API mode, filter options come from `data.items`, component-example slots should default to `/api/component-props/:componentKey` and map `rows[0].props` with `dataBinding.propsObjectField: 'props'`, and static options or component props must not remain as runtime fallback display data. Register custom API/provider resolvers in `src/dataSources/registry.ts` for complex providers or filter-driven scenario data that cannot be represented by plain JSON rows. Do not add `src/widgets/*Data.ts`, `src/data/*.ts`, or other TS fixture modules for mock rows.
...
14. Start with `npm run dev:auto -- --port <port>` or `npm run preview:auto -- --port <port>` when a preview is needed. Use `npm run dev:mock -- --port <port> --mock-port <api-port>` when validating `filters[].source` or API-backed component slots.

## Report Development Terms

- 框架模板: the selected configurable template shell and runtime stack.
- 页面布局配置: `layoutSectionMap`, `layoutRows`, readable `layoutCoordinateMap`, plus page/nav widget wiring. `layoutSectionMap` splits every page into readable exact `12*K` parts such as `12*3 + 12*3 + 12*3` before raw machine rows, and every visible top-level block records `rowSpan >= 3`.
- 分块配置: the block-level config created by `createBlockAreaConfig` and rendered by `BaseLayoutSpan.vue`. It owns `componentSlotContracts` and only the standard block areas `1-1 titleArea` 标题区, `1-2 pillArea` 胶囊按钮区, `3 componentArea` 组件区, and `4 summaryArea` 说明区. `componentRegionPattern` is only an internal/compatibility descriptor derived from the selected template.
- 组件示例: the implemented component's internal content area only, packaged as an independently reusable Vue file. It may fill a component slot inside `3 componentArea`, but it must not carry parent-block filters, controls, title pills, description/help text, or summary copy. It renders as a rounded rectangle without border lines, may expose a removable `20px` centered top title strip, and may include component-owned internal unit and auxiliary metrics such as `unit` and `auxMetrics` when declared by the registered example; hide the title strip when the parent 分块配置 has only one component slot.

## Report Implementation Flow

Use `configurable-zero-to-one-flow.md` as the authoritative no-break handoff contract. For weak-model implementation, create or validate `docs/template-build-packet.md` from `template-build-packet-contract.md` first, then consume packet rows in the sequence below.

1. Select the 框架模板.
2. Design the 页面布局配置.
3. Based on 页面布局配置, create one block area config for each block with `createBlockAreaConfig` and declare `slotCount`, `componentSlotPattern` (`A`, `AB`, `AAB`, `AABBCC`, etc.), and `slotCoordinateList`.
4. Configure the 分块配置 `1-1 titleArea`: title and title style.
...
6. Based on the selected 分块配置 slot configuration, inspect `references/component-examples/config.ts`, `src/widgets/templates/component-examples/` with its README/catalog, and `src/report-template-assets/blueprint/widget-config-schemas.ts`, then choose suitable registered 组件示例 for `3 componentArea`; every component example row maps one declared `slotCoordinate` plus slot pattern code, component slot size, and visual-type size compatibility evidence to a concrete template ID and sample/source evidence. Component-level title, unit, auxiliary metrics, chart/table/list body, and overflow behavior are owned by the registered component example contract.
7. Configure `4 summaryArea`: when no conclusion card/component exists, add text-only/narrative conclusion, note, caveat, or explanation when needed. When a conclusion card/component exists, record `summaryAreaConfig: null` or use `4 summaryArea` only for non-conclusion content such as scope, source, caveat, definition, or action note. For MultiSlot blocks, place conclusion cards or primary conclusion components in the first component slot when componentized conclusions are needed.

Required chain: `frameworkTemplateId -> templateAssetUnderstandingMap -> pageLayoutConfig -> filterSurfaceMap -> toolbarActionMap -> interactionBehaviorMap -> blockAreaConfigMap -> titleAreaConfig -> pillAreaConfig -> componentExampleConfigMap -> summaryAreaConfig`. The chain materializes and configures the selected framework template project; unsupported shell/layout needs become blockers or template backlog items. `pageLayoutConfig` includes `layoutSectionMap`, an exact-12-column, over-12 rejection, minimum-8-row `layoutRows` audit, and visible top-level block `rowSpan >= 3` proof plus `layoutCoordinateMap`; `layoutSectionMap` splits pages into exact `12*K` parts and local A/B previews must be disambiguated in final machine rows. `pageLayoutConfig` and `blockAreaConfigMap` preserve PRD section 4A `RTP-*` / `PATH-*` source and applicable section 4B `ESG-*` / `SEV-*` / `ACT-*` / `TRUST-*` / `MEET-*` gate IDs for every visible block. `blockAreaConfigMap` records `blockCoordinate` (`R-B`), the block area config created by `createBlockAreaConfig`, asset availability, SingleSlot/MultiSlot rationale, `slotCount`, `componentSlotPattern`, and `slotCoordinateList` for every block. `componentExampleConfigMap` records `slotCoordinate` (`R-B-S`), slot pattern code, component slot size, visual-type size compatibility evidence, and component-owned title/unit/auxMetrics config for every `3 componentArea` slot when the selected example declares those props; for example `2-2-1` means the first component slot inside the second block of layout section/page region 2. Only `1-1 titleArea` and `3 componentArea` are always required; `1-2 pillArea` and `4 summaryArea` are optional and can be `null` with a reason. The internal area codes such as `1-2 pillArea` are not page coordinates; standard area targets use `blockCoordinate + areaName`, such as `2-2:pillArea`. When a component slot needs custom chart work, create `selfDevelopmentExceptionMap` entries with `type: componentExample` and register those standalone Vue files inside the configured project before treating the slots as filled.

For low-freedom implementation, the same chain must also exist in the Template Build Packet. Do not edit `dashboard.config.ts`, data files, resolver files, widgets, or registered component examples for a page/block/slot/control that is absent from the packet.
...
- Block slot structure -> block area config created by `createBlockAreaConfig` plus parent block config (`componentSlotContracts`, optional derived `componentRegionPattern`, parent block `displayTitle`/title style, `titlePills`, `bodySummary`) using the standard block areas `1-1`, `1-2`, `3`, and `4`. Every block map row must expose `slotCount`, `componentSlotPattern`, and `slotCoordinateList`. Component-owned `unit` and `auxMetrics` stay inside the registered component example props/config, not on the parent block.
- Block pill runtime binding -> keep pills in `1-2 pillArea`, then use `titlePills[].filters`, `titlePills[].params`, `titlePills[].props`, `titlePills[].dataBinding`, and optional `titlePills[].actions` when the pill changes data, display fields, or interaction state. The shell exposes the active pill through `$context.activeTitlePillId`, `$context.activeTitlePillLabel`, and `$context.activeTitlePill`; pill filters join the scoped filters, pill params join data-source params, and pill props/dataBinding are applied by `BaseLayoutSpan.vue` to the active component examples.
- Component slot fill -> registered 组件示例 only, inside `3 componentArea`. Mount or copy the selected standalone component example Vue file, or use a `componentExampleId` that maps to that file; record `slotCoordinate`, slot pattern code, component slot size, visual-type size compatibility evidence from `widget-config-schemas.ts`, copy source/target, sample/source evidence, props/data/state contract, and data binding. Do not attach parent title/pills/filters/controls/description/help/summary to the slot. Pass slot count/title props so multi-slot blocks may show the optional `20px` component example title strip, and single-slot blocks hide it. Keep component-owned internal props such as `unit` and `auxMetrics` when the selected example supports them. Text/prose/placeholder content, `visualType` alone, or an inline widget object does not fill a component slot.
- Component slot runtime binding -> use `componentSlots[].data`, `componentSlots[].filterScope`, `componentSlots[].dataBinding`, and `componentSlots[].actions` through the owning widget config. The shell loads slot data separately with `blockId + slotId`, passes `slotData` and `slotContext` into `BaseLayoutSpan`, then maps rows into registered component-example props. This is the supported route for binding existing component examples to template filters; do not add component-example-specific filter panels or duplicate data loaders inside the slot.
- ECharts fallback component body -> standalone 组件示例 Vue file first, then slot fill. Do not implement fallback chart markup directly inside a 分块配置 or parent block.
...
- Configured popup/jump/drilldown -> `actions` on the block widget or component slot. The shell resolves `dashboard-action` events, tries `src/actions/registry.ts` custom handlers first, and otherwise executes built-in `route`, `external`, `drawer`/`drilldown`, `modal`, `popover`/`popup`, `cross-filter`, `fullscreen`, and `export` behavior. Use `src/actions/registry.ts` only for behavior that needs custom permission, API orchestration, analytics, or a business-specific overlay.
- Code change ledgers -> same-directory sidecars under `__change_logs__/<code-file-name>.changes.md`. Run `npm run ledger:init` before the first source edit to create baseline sidecars, run `npm run ledger:check` before handoff, read before each scoped source edit, and append after each scoped source edit. Do not store file-level change history only in chat, commit messages, PR descriptions, broad delivery indexes, or a legacy `change_logs` folder. The expected directory name is `__change_logs__`.
...
- Component-owned title/control areas are allowed only for ordinary business widgets outside the current 组件示例 slot contract. A 组件示例 inside `3 componentArea` may show its optional removable title strip plus body content, including declared internal auxiliary metrics; `localFilters[]`, filter-panel triggers, and lightweight links belong to non-slot widgets, shell/page config, or an explicit template redesign.
...
- `localFilters[]` display rules apply only to ordinary business widgets that are not used as 组件示例 slot fills. Registered component examples must not render local filters or filter-panel triggers.
